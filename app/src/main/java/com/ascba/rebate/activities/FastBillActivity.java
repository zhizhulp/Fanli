...
public class FastBillActivity extends BaseBillActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setBillNetworker(new BillNetworker() {
            ...
            public void executeNetworkRefresh() {
                requestNetwork(UrlUtils.payCashLog,0);
            }
            @Override
            public void parseDataAndRefreshUI(int what, JSONObject dataObj, String message) {
                if(what==0){
                    JSONArray array = dataObj.optJSONArray("pay_cash_log");
                    if(array!=null && array.length()!=0){
                        for (int i = 0; i < array.length(); i++) {
                            ...
                            String score = obj.optString("cashing_money_num");
