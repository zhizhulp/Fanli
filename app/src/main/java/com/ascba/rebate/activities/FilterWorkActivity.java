...
public class FilterWorkActivity extends BaseNetWorkActivity implements View.OnClickListener, ShopABarText.Callback {
    ...
    private void initAttrsData(List<GoodsAttr> gas) {
        ...
        strs1.add(ga1.new Attrs(1,"自营店铺", 0));
        strs1.add(ga1.new Attrs(1,"商家店铺", 0));
        ga1.setTitle("店铺类型：");
        ga1.setStrs(strs1);
        gas.add(ga1);
        ...
        for (int i = 0; i < 5; i++) {
            if (i == 0) {
                ...
                for (int j = 0; j < 3; j++) {
                    strs.add(ga.new Attrs(1,"红色/白色", 0));
                }
                ga.setTitle("颜色分类：");
                ga.setStrs(strs);
                gas.add(ga);
            ...
            if (i == 1) {
                ...
                for (int j = 0; j < 15; j++) {
                    ...
                    strs.add(ga.new Attrs(1,(40 + j + 0.5) + "", 0));
                }
                ga.setTitle("鞋码：");
                ga.setStrs(strs);
                gas.add(ga);
            ...
            if (i == 2) {
                ...
                GoodsAttr ga = new GoodsAttr(FilterAdapter.TYPE1, R.layout.filter_layout);
                for (int j = 0; j < 3; j++) {
                    strs.add(ga.new Attrs(1,"方形" + i, 0));
                }
                ga.setTitle("其他分类：");
                ga.setStrs(strs);
                gas.add(ga);
